...
export class BattleSystem {
    ...
    bindEvents() {
        this._fxText = (e) => this.spawnText(e.detail.x, e.detail.y, e.detail.text, e.detail.color, e.detail.size || 20);
        this._fxShake = (e) => this.shake = Math.max(this.shake, e.detail);
        this._fxParticle = (e) => this.parts.push(e.detail);
        this._fxLightning = (e) => this.objs.push({ type: 'lightning', x1: e.detail.x1, y1: e.detail.y1, x2: e.detail.x2, y2: e.detail.y2, life: 10 });
        this._fxFinal = (e) => this.showFinalText(e.detail.text, e.detail.color);
        this._fxDamage = (e) => {
            const d = e.detail;
            for (let i = 0; i < 8; i++) this.parts.push({ x: d.x, y: d.y, vx: (Math.random() - 0.5) * 15, vy: (Math.random() - 0.5) * 15, life: 25, color: '#fff', size: 4 });
            this.spawnText(d.x, d.y - 20, `-${d.amount.toFixed(0)}`, '#f00');
        };
        this._cast = (e) => this.handleCast(e.detail.caster, e.detail.enemy, e.detail.isFinal, e.detail.skillName);
        window.addEventListener('fx:text', this._fxText);
        window.addEventListener('fx:shake', this._fxShake);
        window.addEventListener('fx:particle', this._fxParticle);
        window.addEventListener('fx:lightning', this._fxLightning);
        window.addEventListener('fx:finalText', this._fxFinal);
        window.addEventListener('fx:damage', this._fxDamage);
        window.addEventListener('battle:cast', this._cast);
    }
    start() {
        this.running = true;
        this.loop();
    ...
    stop() {
        this.running = false;
        window.removeEventListener('fx:text', this._fxText);
        window.removeEventListener('fx:shake', this._fxShake);
        window.removeEventListener('fx:particle', this._fxParticle);
        window.removeEventListener('fx:lightning', this._fxLightning);
        window.removeEventListener('fx:finalText', this._fxFinal);
        window.removeEventListener('fx:damage', this._fxDamage);
        window.removeEventListener('battle:cast', this._cast);
    }
    spawnText(x, y, text, color, size = 20) { this.texts.push({ x, y, text, color, life: 50, size }); }
    ...
    handleCast(caster, enemy, isFinal, skillName) {
        setTimeout(() => {
            if (caster.template.id !== 'gojo' && caster.template.id !== 'yin') caster.state = 'idle';
            else if (!caster.mugenActive) caster.state = 'idle';
            const id = caster.template.id;
            const ang = Math.atan2(enemy.y - caster.y, enemy.x - caster.x);
            const spX = caster.x + Math.cos(ang) * 45; const spY = caster.y + Math.sin(ang) * 45;
            if (isFinal) {
                ...
                else if (id === 'giorno') { document.getElementById('domain-overlay').style.background = 'rgba(255,215,0,0.3)'; document.getElementById('domain-overlay').style.opacity = 1; enemy.x = 500; enemy.y = 300; enemy.vx = 0; enemy.vy = 0; enemy.mana = 0; this.spawnText(500, 300, 'RETURN TO ZERO', '#FFD700', 60); setTimeout(() => { document.getElementById('domain-overlay').style.opacity = 0; enemy.takeDamage(200, caster); }, 2000); }
                else if (id === 'garou') { this.shake = 80; this.fireBeam(caster, spX, spY, enemy, '#4B0082', '#000', 150); enemy.takeDamage(250, caster); }
                else if (id === 'kaguya') { this.objs.push({ type: 'omega_explosion', x: enemy.x, y: enemy.y, r: 10, maxR: 900, life: 200, color: '#000', owner: caster, dmg: 300 }); }
                else if (id === 'featherine') { enemy.hp = 1; this.spawnText(enemy.x, enemy.y, 'SCRIPT REWRITTEN: HP=1', '#A020F0'); }
                else if (id === 'wukong') { for (let i = 0; i < 4; i++) { let clone = new Fighter(caster.template, caster.level, caster.isPlayer); clone.isClone = true; clone.hp = caster.maxHp; clone.x = caster.x + (Math.random() - 0.5) * 100; clone.y = caster.y + (Math.random() - 0.5) * 100; clone.vx = (Math.random() - 0.5) * 10; clone.vy = (Math.random() - 0.5) * 10; if (caster.isPlayer) this.p1_allies.push(clone); else this.p2_allies.push(clone); } }
                else if (id === 'yin') { if (skillName === 'DIVINE VOID') { document.getElementById('domain-overlay').style.background = '#000'; document.getElementById('domain-overlay').style.opacity = 1; this.shake = 100; setTimeout(() => { document.getElementById('domain-overlay').style.opacity = 0; enemy.takeDamage(999, caster); }, 1000); } else { this.objs.push({ type: 'omega_explosion', x: enemy.x, y: enemy.y, r: 10, maxR: 600, life: 100, color: '#000', owner: caster, dmg: 200 }); } }
                else if (id === 'antimonitor') { this.shake = 60; this.objs.push({ type: 'antimatter_wave', owner: caster, x: 0, y: 0, w: 1000, h: 600, life: 60 }); enemy.takeDamage(300, caster); }
                else if (id === 'sukuna') { document.getElementById('domain-overlay').style.background = 'rgba(100,0,0,0.3)'; document.getElementById('domain-overlay').style.opacity = 1; for (let i = 0; i < 40; i++) { setTimeout(() => { this.objs.push({ type: 'cleave_hd', owner: caster, x: enemy.x + (Math.random() - 0.5) * 120, y: enemy.y + (Math.random() - 0.5) * 120, vx: 0, vy: 0, angle: Math.random(), life: 15, sureHit: true, color: '#000' }); enemy.takeDamage(20, caster); this.shake = 5; }, i * 30); } setTimeout(() => document.getElementById('domain-overlay').style.opacity = 0, 2500); }
                ...
                else if (id === 'broly') { this.shake = 100; for (let i = 0; i < 6; i++) { setTimeout(() => { this.objs.push({ type: 'broly_meteor', x: Math.random() * 1000, y: Math.random() * 600, r: 10, maxR: 500, life: 100, color: '#0f0', owner: caster, dmg: 150 }); }, i * 250); } }
                ...
                else { this.objs.push({ type: 'omega_explosion', x: enemy.x, y: enemy.y, r: 10, maxR: 500, life: 100, color: caster.template.glow, owner: caster, dmg: 150 }); }
                return;
            }
            // Normal skills
            if (id === 'hulk') {
                if (skillName === 'THUNDERCLAP') { this.shake = 20; enemy.takeDamage(50, caster); this.objs.push({ type: 'gamma_wave', x: caster.x, y: caster.y, r: 10, maxR: 300, life: 50, color: '#0f0', owner: caster }); }
                else { for (let i = 0; i < 5; i++) setTimeout(() => { this.objs.push({ type: 'debris', x: enemy.x, y: enemy.y - 300, vx: (Math.random() - 0.5) * 5, vy: 15 + Math.random() * 10, life: 60, owner: caster }); }, i * 100); }
            } else if (id === 'mahoraga') {
                ...
                else for (let i = 0; i < 10; i++) setTimeout(() => enemy.takeDamage(6, caster), i * 40);
            } else if (id === 'garou') {
                ...
                else { enemy.x = Math.random() * 1000; enemy.y = Math.random() * 600; this.spawnText(enemy.x, enemy.y, 'DIMENSION SHIFT', '#000'); enemy.takeDamage(40, caster); }
                ...
                if (caster.stage === 2) {
                    if (skillName === 'PLANET CRUSHER') this.objs.push({ type: 'omega_explosion', x: spX, y: spY, r: 10, maxR: 250, life: 100, color: '#0f0', owner: caster, dmg: 120 });
                ...
                if (skillName === 'SIXFOLD') {
                    ...
                    let dashInt = setInterval(() => {
                        ...
                        window.dispatchEvent(new CustomEvent('fx:lightning', { detail: { x1: caster.x, y1: caster.y, x2: enemy.x, y2: enemy.y } }));
                        if (caster.dashSteps <= 0) { clearInterval(dashInt); caster.state = 'idle'; caster.x = enemy.x - 50; caster.y = enemy.y; enemy.takeDamage(60, caster); }
                    }, 80);
                ...
                else { this.shake = 30; enemy.takeDamage(50, caster); this.spawnText(enemy.x, enemy.y, 'TABLE FLIP', '#fff'); }
                ...
                else { enemy.takeDamage(30, caster); this.shake = 10; }
            } else this.proj(caster, spX, spY, enemy, 'generic');
        }, 600);
    }
    fireBeam(o, sx, sy, t, c, g, w) {
        let a = Math.atan2(t.y - sy, t.x - sx);
        this.objs.push({ type: 'beam', sx: sx, sy: sy, ex: sx + Math.cos(a) * 1000, ey: sy + Math.sin(a) * 1000, c: c, g: g, w: w, life: 20, owner: o });
        t.takeDamage(50, o); this.shake = 15;
    }
    proj(o, x, y, t, type) { let a = Math.atan2(t.y - y, t.x - x); this.objs.push({ type: 'proj', k: type, x: x, y: y, vx: Math.cos(a) * 18, vy: Math.sin(a) * 18, o: o, life: 60 }); }
    loop() {
        ...
        [...this.p1_allies, ...this.p2_allies].forEach(a => {
            if (a.state === 'zenitsu_dash' && a.pathQueue) {
                if (a.pathQueue.length > 0) { let target = a.pathQueue[0]; a.x += (target.x - a.x) * 0.3; a.y += (target.y - a.y) * 0.3; window.dispatchEvent(new CustomEvent('fx:lightning', { detail: { x1: a.x, y1: a.y, x2: a.x + (Math.random() - 0.5) * 100, y2: a.y + (Math.random() - 0.5) * 100 } })); if (Math.hypot(a.x - target.x, a.y - target.y) < 20) a.pathQueue.shift(); }
                else { a.state = 'idle'; let enemies = this.p1_allies.includes(a) ? this.p2_allies : this.p1_allies; enemies.forEach(e => e.takeDamage(50, a)); this.shake = 20; }
            }
            a.update(this.p1_allies.includes(a) ? this.p2 : this.p1);
        });
        this.p1_allies.forEach(a1 => {
            this.p2_allies.forEach(a2 => {
                if (a1.state !== 'dashing' && a2.state !== 'dashing') {
                    ...
                    if (dist < a1.r + a2.r) { let ang = Math.atan2(dy, dx); let v = 5; a1.vx = -Math.cos(ang) * v; a1.vy = -Math.sin(ang) * v; a2.vx = Math.cos(ang) * v; a2.vy = Math.sin(ang) * v; a1.takeDamage(2, a2); a2.takeDamage(2, a1); }
                }
                if (a1.state === 'dashing' && Math.hypot(a1.x - a2.x, a1.y - a2.y) < 60) a2.takeDamage(40, a1);
                if (a2.state === 'dashing' && Math.hypot(a2.x - a1.x, a2.y - a1.y) < 60) a1.takeDamage(40, a2);
            });
        });
        ...
        for (let i = this.objs.length - 1; i >= 0; i--) {
            ...
            else if (o.type === 'gamma_wave') { this.ctx.strokeStyle = '#0f0'; this.ctx.lineWidth = 5; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, o.r, 0, Math.PI * 2); this.ctx.stroke(); o.r += 8; if (o.r > o.maxR) o.life = 0; enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < o.r + 20 && Math.hypot(o.x - e.x, o.y - e.y) > o.r - 20) { e.takeDamage(8, o.owner); e.vx += (e.x - o.x) * 0.2; e.vy += (e.y - o.y) * 0.2; } }); }
            else if (o.type === 'debris') { o.x += o.vx; o.y += o.vy; this.ctx.fillStyle = '#555'; this.ctx.fillRect(o.x, o.y, 25, 25); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 30) { e.takeDamage(50, o.owner); o.life = 0; } }); }
            else if (o.type === 'dharma_slash') { o.x += o.vx; o.y += o.vy; this.ctx.strokeStyle = '#FFD700'; this.ctx.lineWidth = 4; this.ctx.shadowBlur = 10; this.ctx.shadowColor = '#fff'; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 45, o.angle - 0.6, o.angle + 0.6); this.ctx.stroke(); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 45) { let d = o.dmg || 30; if (o.ignoreDef) { d *= 1.5; this.spawnText(e.x, e.y, 'PIERCED', '#f00'); } e.takeDamage(d, o.owner); o.life = 0; } }); }
            else if (o.type === 'flash_vortex') { this.ctx.strokeStyle = '#ff0'; this.ctx.lineWidth = 2; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 100, 0, Math.PI * 2); this.ctx.stroke(); enemies.forEach(e => { e.x += (o.x - e.x) * 0.1; e.y += (o.y - e.y) * 0.1; if (Math.hypot(o.x - e.x, o.y - e.y) < 100) e.takeDamage(3, o.owner); }); }
            else if (o.type === 'antimatter_wave') { o.x += 20; this.ctx.fillStyle = 'rgba(255,255,255,0.8)'; this.ctx.fillRect(o.x, 0, 50, 600); this.ctx.fillStyle = 'rgba(0,0,0,0.5)'; this.ctx.fillRect(o.x - 20, 0, 20, 600); enemies.forEach(e => { if (Math.abs(o.x - e.x) < 50) e.takeDamage(5, o.owner); }); }
            else if (o.type === 'yin_yang_proj') { o.x += o.vx; o.y += o.vy; this.ctx.fillStyle = o.color; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 15, 0, Math.PI * 2); this.ctx.fill(); this.ctx.strokeStyle = o.color === '#fff' ? '#000' : '#fff'; this.ctx.stroke(); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 30) { e.takeDamage(35, o.owner); o.life = 0; } }); }
            ...
            else if (o.type === 'tracking_proj') { let dx = o.target.x - o.x, dy = o.target.y - o.y, d = Math.sqrt(dx * dx + dy * dy); o.x += dx / d * o.speed; o.y += dy / d * o.speed; this.ctx.fillStyle = o.color; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 8, 0, Math.PI * 2); this.ctx.fill(); if (d < 30) { o.target.takeDamage(30, o.owner); o.life = 0; this.objs.push({ type: 'omega_explosion', x: o.x, y: o.y, r: 10, maxR: 100, life: 20, color: o.color }); } }
            else if (o.type === 'broly_meteor') { this.ctx.fillStyle = '#0f0'; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 40, 0, Math.PI * 2); this.ctx.fill(); if (o.life < 10) { this.objs.push({ type: 'omega_explosion', x: o.x, y: o.y, r: 10, maxR: 400, life: 60, color: '#0f0', owner: o.owner, dmg: 200 }); this.shake = 40; } }
            ...
            else if (o.type === 'skull') { o.x += o.vx; o.y += o.vy; this.ctx.fillStyle = '#ccc'; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 10, 0, Math.PI * 2); this.ctx.fill(); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 30) { e.takeDamage(15, o.owner); o.owner.hp = Math.min(o.owner.maxHp, o.owner.hp + 5); o.life = 0; } }); }
            else if (o.type === 'omega_explosion') { o.r += (o.maxR - o.r) * 0.15; this.ctx.fillStyle = o.color || '#0f0'; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, o.r, 0, Math.PI * 2); this.ctx.globalAlpha = o.life / 40; this.ctx.fill(); if (o.dmg) enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < o.r) e.takeDamage(2, o.owner); }); }
            else if (o.type === 'proj') { o.x += o.vx; o.y += o.vy; this.ctx.fillStyle = o.k === 'purple' ? '#a020f0' : (o.k === 'bullet' ? '#fff' : '#ff8c00'); this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 12, 0, Math.PI * 2); this.ctx.fill(); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 30) { e.takeDamage(25, o.owner); o.life = 0; } }); }
            else if (o.type === 'leviathan_axe') { o.x += o.vx; o.y += o.vy; o.angle += 0.5; this.ctx.save(); this.ctx.translate(o.x, o.y); this.ctx.rotate(o.angle); this.ctx.fillStyle = '#8B4513'; this.ctx.fillRect(-3, -20, 6, 40); this.ctx.fillStyle = '#C0C0C0'; this.ctx.shadowBlur = 15; this.ctx.shadowColor = '#00ffff'; this.ctx.beginPath(); this.ctx.moveTo(0, -5); this.ctx.lineTo(20, -15); this.ctx.quadraticCurveTo(25, 0, 20, 15); this.ctx.lineTo(0, 5); this.ctx.fill(); this.ctx.beginPath(); this.ctx.moveTo(0, -5); this.ctx.lineTo(-20, -15); this.ctx.quadraticCurveTo(-25, 0, -20, 15); this.ctx.lineTo(0, 5); this.ctx.fill(); this.ctx.restore(); this.parts.push({ x: o.x, y: o.y, vx: 0, vy: 0, life: 5, color: '#00ffff', size: 3 }); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 35) { e.takeDamage(35, o.owner); o.life = 0; this.shake = 10; } }); }
            ...
            else if (o.type === 'black_whip') { this.ctx.strokeStyle = '#006400'; this.ctx.lineWidth = 3; this.ctx.beginPath(); this.ctx.moveTo(o.owner.x, o.owner.y); let mx = (o.owner.x + o.target.x) / 2 + (Math.random() - 0.5) * 50; let my = (o.owner.y + o.target.y) / 2 + (Math.random() - 0.5) * 50; this.ctx.lineTo(mx, my); this.ctx.lineTo(o.target.x, o.target.y); this.ctx.stroke(); if (o.life % 5 === 0) o.target.takeDamage(10, o.owner); }
            else if (o.type === 'clone') { let dx = o.target.x - o.x, dy = o.target.y - o.y, d = Math.hypot(dx, dy); o.x += dx / d * 8; o.y += dy / d * 8; this.ctx.fillStyle = '#ffd700'; this.ctx.globalAlpha = 0.6; this.ctx.beginPath(); this.ctx.arc(o.x, o.y, 15, 0, Math.PI * 2); this.ctx.fill(); if (d < 30) { o.target.takeDamage(20, o.owner); o.life = 0; } }
            ...
            else if (o.type === 'mugetsu_wave') { o.x += o.vx; o.y += o.vy; this.ctx.save(); this.ctx.translate(o.x, o.y); this.ctx.rotate(o.angle); this.ctx.fillStyle = '#000'; this.ctx.shadowBlur = 20; this.ctx.shadowColor = '#00f'; this.ctx.beginPath(); this.ctx.ellipse(0, 0, 70, 20, 0, 0, Math.PI * 2); this.ctx.fill(); this.ctx.strokeStyle = '#00f'; this.ctx.lineWidth = 3; this.ctx.stroke(); this.ctx.restore(); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 70) { e.takeDamage(90, o.owner); o.life = 0; this.shake = 30; this.objs.push({ type: 'omega_explosion', x: o.x, y: o.y, r: 10, maxR: 250, life: 40, color: '#000', owner: o.owner, dmg: 50 }); } }); }
            else if (o.type === 'slash_proj') { o.x += o.vx; o.y += o.vy; this.ctx.strokeStyle = o.color || '#f00'; this.ctx.lineWidth = 2; this.ctx.beginPath(); this.ctx.moveTo(o.x, o.y); this.ctx.lineTo(o.x - o.vx * 2, o.y - o.vy * 2); this.ctx.stroke(); enemies.forEach(e => { if (Math.hypot(o.x - e.x, o.y - e.y) < 30) { e.takeDamage(15, o.owner); o.life = 0; } }); }
        ...
        if (this.p1.hp <= 0 || this.p2.hp <= 0) {
            ...
            const loser = this.p1.hp > 0 ? this.p2 : this.p1;
            ...
            const xpEarned = Math.floor(loser.level * 50 + 100);
            const coinsEarned = isPlayerWin ? 100 : 20;
            this.game.db.saveBattleResult(isPlayerWin, xpEarned, coinsEarned).then(() => {
                console.log(`[Battle] Resultado salvo. Vitória=${isPlayerWin}, XP=${xpEarned}, Coins=${coinsEarned}`);
            });
            return;
        }
