export class Fighter {
    ...
    getScaledSpeed() {
        return this.template.speed * (1 + (this.level - 1) * 0.02);
    }
    getScaledDamage() {
        return (this.template.damage || 30) * (1 + (this.level - 1) * 0.05);
    }
    update(enemy, battleSystem) {
        ...
        if (this.state !== 'cast' && !this.isClone) {
            this.mana = Math.min(100, this.mana + (100 / this.template.cd) * 4 * (1 + (this.level - 1) * 0.03));
        ...
        if (this.flashMode) {
            ...
            battleSystem.parts.push({ x: this.x, y: this.y, vx: 0, vy: 0, life: 10, color: '#ff0', size: 8 });
        }
