 ...
 * Menu.js
 * 
 * Responsabilidade: Renderizar e gerenciar a UI do menu principal.
 * Regra: Não contém lógica de negócio. Apenas exibe o que o Game manda.
 */
...
export class Menu {
    ...
    renderGrid(playerRoster) {
        ...
        // Mapa de personagens desbloqueados
        ...
        ROSTER.forEach(char => {
            ...
            el.innerHTML = `
                ...
                <div class="rarity rarity-${char.rarity || 'common'}">${(char.rarity || 'common').toUpperCase()}</div>
                ${isUnlocked ? `<small style="color:#888">Lv.${level}</small>` : '<small style="color:#555">Bloqueado</small>'}
            `;

            if (isUnlocked) {
                el.addEventListener('click', () => this.toggleSelection(char, el));
            }
    ...
    toggleSelection(char, el) {
        if (this.selected.includes(char)) {
            this.selected = this.selected.filter(c => c !== char);
    ...
    updatePlayButton() {
        if (this.selected.length === 2) {
            ...
            this.playBtn.textContent = 'INICIAR BATALHA';
        } else {
            this.playBtn.disabled = true;
            this.playBtn.textContent = 'SELECIONE 2';
        }
    }
    ...
    onPlayClick() {
        ...
        // Pega os levels reais do jogador do Firestore
